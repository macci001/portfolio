...
/* ─────────────────────────────────────────────────────────────
   YOUTUBE IFRAME API — auto-pause previous player
───────────────────────────────────────────────────────────── */
...
// Loads the YT IFrame API script once, then resolves all queued callbacks.
...
function YTEmbed({ yId, onPlay }: YTEmbedProps) {
  const divRef = useRef<HTMLDivElement>(null);
  const playerRef = useRef<YT.Player | null>(null);
  // Keep a stable ref to onPlay so the closure inside useEffect doesn't go stale
        ...
        events: {
          onStateChange: (e: YT.OnStateChangeEvent) => {
            if (e.data === window.YT.PlayerState.PLAYING && playerRef.current) {
              onPlayRef.current(playerRef.current);
            }
          },
        },
      });
...
const FILMS: Film[] = [
  {
    ...
    y_id: "ysAG3JiQaX0",
    about:
      "Wore multiple hats — scouting and locking filming locations, supporting on-set production logistics, and performing as the film's voiceover artist.",
    insight:
  ...
  },
  {
    ...
    y_id: "aoE3UZR80F4",
    about:
      "Responsible for managing the lighting of scenes and maintaining continuity across takes — ensuring consistent visual quality throughout the shoot.",
    insight:
  ...
  },
  {
    ...
    y_id: "8FUJPhwIZ_k",
    about:
      "Performed as an on-screen actor, bringing a character to life through presence, timing, and physicality on a short film set.",
    insight:
  ...
  return (
    <>
      ...
      >
        <button
          onClick={() => go("hero")}
          style={{ background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: F.playfair, fontSize: isMobile ? "1rem" : "1.1rem", color: "#fff" }}
        >
          {OWNER.firstName}
        </button>
          ...
          {links.map(({ label, id }) => (
            <button
              ...
              style={{
                ...
                fontFamily: F.dm, fontSize: "0.9rem", letterSpacing: "0.08em", textTransform: "uppercase",
                textAlign: "left", color: active === id ? "#F59E0B" : "rgba(255,255,255,0.6)",
              }}
            >
              {label}
            </button>
  ...
  return (
    <section
      ...
      style={{
        ...
        padding: isMobile ? "5rem 1.5rem 4rem" : "6rem 2rem 5rem", textAlign: "center", position: "relative", overflow: "hidden",
      }}
    >
      <div
        style={{
          position: "absolute", top: "38%", left: "50%", transform: "translate(-50%,-50%)",
          width: isMobile ? "400px" : "800px", height: isMobile ? "400px" : "800px",
          borderRadius: "50%", background: "radial-gradient(circle, rgba(245,158,11,0.07) 0%, transparent 68%)",
          pointerEvents: "none",
        }}
      />
      ...
      <div style={{ display: "flex", gap: "0.85rem", flexWrap: "wrap", justifyContent: "center", animation: "fadeUp 0.6s 0.34s ease both", opacity: 0 }}>
        <button
          onClick={() => smoothScroll("ads")}
          style={{ background: "#F59E0B", color: "#000", border: "none", cursor: "pointer", fontFamily: F.dm, fontWeight: 700, fontSize: isMobile ? "0.75rem" : "0.8rem", letterSpacing: "0.08em", textTransform: "uppercase", padding: isMobile ? "0.8rem 1.8rem" : "0.9rem 2.2rem", borderRadius: "2px" }}
        >
          View Ads ↓
        </button>
        <button
          onClick={() => smoothScroll("films")}
          style={{ background: "transparent", color: "#fff", cursor: "pointer", border: "1px solid rgba(255,255,255,0.25)", fontFamily: F.dm, fontWeight: 500, fontSize: isMobile ? "0.75rem" : "0.8rem", letterSpacing: "0.08em", textTransform: "uppercase", padding: isMobile ? "0.8rem 1.8rem" : "0.9rem 2.2rem", borderRadius: "2px" }}
        >
          View Films ↓
        </button>
      </div>
...
function Tag({ label, accent }: { label: string; accent?: string }) {
  return (
    <span
      style={{
        fontFamily: F.dm, fontSize: "0.65rem", letterSpacing: "0.06em", textTransform: "uppercase",
        color: accent || "rgba(255,255,255,0.4)",
        border: `1px solid ${accent ? accent + "55" : "rgba(255,255,255,0.12)"}`,
        padding: "3px 9px", borderRadius: "2px", display: "inline-block",
      }}
    >
      {label}
    </span>
...
function SectionHeader({ num, title, sub, bp }: { num: string; title: string; sub: string; bp: Breakpoint }) {
  const isMobile = bp === "mobile";
  return (
...
function AdCard({ ad, index, bp, onPlay }: AdCardProps) {
  ...
  return (
    <article
      style={{
        ...
        animation: `fadeUp 0.55s ${0.05 + index * 0.07}s ease both`, opacity: 0,
      }}
    >
      <YTEmbed yId={ad.y_id} onPlay={onPlay} />

      <div style={{ padding: isMobile ? "1.3rem 1.25rem" : "1.6rem 1.8rem", display: "flex", flexDirection: "column", gap: "1rem", flexGrow: 1 }}>
        <div>
          <p style={{ fontFamily: F.dm, fontSize: "0.65rem", letterSpacing: "0.14em", textTransform: "uppercase", color: ad.accent, marginBottom: "0.3rem" }}>
            {ad.badge} &nbsp;{ad.brand}
          </p>
        ...
        <div
          style={{
            ...
            border: `1px solid ${ad.accent}33`, borderLeft: `4px solid ${ad.accent}`,
            borderRadius: "0 8px 8px 0", padding: isMobile ? "0.9rem 1rem" : "1.1rem 1.25rem",
          }}
        >
        ...
        <div>
          <button
            ...
            style={{
              background: "none", border: "1px solid rgba(255,255,255,0.08)", cursor: "pointer", width: "100%",
              textAlign: "left", padding: "0.6rem 0.9rem", borderRadius: "4px",
              display: "flex", alignItems: "center", justifyContent: "space-between",
              fontFamily: F.dm, fontSize: "0.68rem", letterSpacing: "0.08em", textTransform: "uppercase",
              color: "rgba(255,255,255,0.34)", transition: "border-color 0.2s,color 0.2s",
            }}
...
function FilmCard({ film, index, bp, onPlay }: FilmCardProps) {
  const isMobile = bp === "mobile";
  ...
  return (
    <article
      style={{
        background: "#111", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "10px",
        overflow: "hidden", display: "flex", flexDirection: "column",
        animation: `fadeUp 0.55s ${0.05 + index * 0.09}s ease both`, opacity: 0,
      }}
    >
      <YTEmbed yId={film.y_id} onPlay={onPlay} />

      <div style={{ padding: isMobile ? "1.2rem 1.25rem" : "1.4rem 1.7rem", display: "flex", flexDirection: "column", gap: "0.9rem", flexGrow: 1 }}>
        <div>
          <p style={{ fontFamily: F.dm, fontSize: "0.65rem", letterSpacing: "0.14em", textTransform: "uppercase", color: "rgba(255,255,255,0.26)", marginBottom: "0.25rem" }}>
            {film.icon} &nbsp;Short Film
          </p>
          <h3 style={{ fontFamily: F.playfair, fontSize: isMobile ? "1.2rem" : "1.28rem", color: "#fff", margin: "0 0 0.45rem" }}>{film.title}</h3>
          <span
            style={{
              display: "inline-block", fontFamily: F.dm, fontSize: "0.65rem", letterSpacing: "0.06em",
              textTransform: "uppercase", color: film.roleColor, background: film.roleBg,
  ...
  return (
    <section id="contact" style={{ padding: isMobile ? "5rem 0 6rem" : "7rem 0 8rem" }}>
      <div style={{ maxWidth: "720px", margin: "0 auto", padding: "0 1.5rem", textAlign: "center" }}>
        <p style={{ fontFamily: F.dm, fontSize: "0.65rem", letterSpacing: "0.22em", textTransform: "uppercase", color: "#F59E0B", marginBottom: "0.7rem" }}>
          Let&apos;s Work Together
        </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "1rem", marginBottom: "2rem" }}>
          {cards.map(({ icon, label, value, href }) => (
            <a
              key={label}
              href={href}
              style={{ display: "block", background: "#111", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "8px", padding: "1.25rem 1.5rem", textDecoration: "none", transition: "border-color 0.2s,background 0.2s" }}
        ...
        <a
          ...
          style={{ display: "inline-block", background: "#F59E0B", color: "#000", fontFamily: F.dm, fontWeight: 700, fontSize: "0.8rem", letterSpacing: "0.1em", textTransform: "uppercase", padding: "1rem 2.5rem", borderRadius: "2px", textDecoration: "none", transition: "opacity 0.2s" }}
...
export default function Portfolio() {
  ...
  // ── Shared active-player ref ─────────────────────────────
  const activePlayerRef = useRef<YT.Player | null>(null);

  const handlePlay = useCallback((player: YT.Player) => {
    ...
    <>
      ...
      <div style={{ background: "#080808", minHeight: "100vh" }}>
        ...
        <section id="ads" style={{ padding: pad }}>
          <div style={inner}>
            <SectionHeader
              num="Section 01 — Advertisements"
              title="Original Ads"
              sub="Four ads across food, FMCG, skincare, and fashion — each built around a clear strategic intent."
              bp={bp}
            />
            <div
              className="ads-grid"
              style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(min(100%,490px),1fr))", gap: isMobile ? "1.25rem" : "1.75rem" }}
            >
              {ADS.map((ad, i) => (
                <AdCard key={ad.id} ad={ad} index={i} bp={bp} onPlay={handlePlay} />
              ))}
            </div>
          </div>
        </section>
        ...
        <section id="films" style={{ padding: pad }}>
          <div style={inner}>
            <SectionHeader
              num="Section 02 — Short Films"
              title="On-Screen & On-Set Experience"
              sub="Three films, three different roles — actor, production crew, and lighting department."
              bp={bp}
            />
            <div style={{ display: "flex", flexWrap: "wrap", gap: "7px", marginBottom: isMobile ? "2rem" : "3rem" }}>
              {["On-camera performance", "Production coordination", "Location scouting", "Voiceover", "Scene continuity", "Lighting management", "Cross-dept. collaboration"].map((s) => (
                <Tag key={s} label={s} />
              ))}
            </div>
            <div
              className="films-grid"
              style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(min(100%,330px),1fr))", gap: isMobile ? "1.25rem" : "1.5rem", marginBottom: isMobile ? "2rem" : "2.5rem" }}
            >
              {FILMS.map((f, i) => (
                <FilmCard key={f.id} film={f} index={i} bp={bp} onPlay={handlePlay} />
              ))}
            </div>
          </div>
        </section>
        ...
        <footer
          style={{
            borderTop: "1px solid rgba(255,255,255,0.06)", padding: isMobile ? "1.25rem" : "1.5rem 2.5rem",
